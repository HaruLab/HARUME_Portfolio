...
const CREDIT_REGEX = /^(Music Video 制作|ジャケットイラスト|作詞作曲|Music & Lyric|動画制作|動画作者|編集ソフト|使用ソフト|ナレーション|BGM作曲|Inst arrange|Music|Vocal|Mix|Mixing|MIX|Illust|Ilust|Illstration|Illustration|Live2D|Movie|Video|Cast|Director|Model|Motion|Camera|Lyrics|Lyric|Arrangement|Tuning|Mastering|Instrument|作詞|作曲|編曲|歌|ボーカル|ミックス|イラスト|動画|映像|制作|出演|モデル|Original|Credit|Bass|Guitar|Drums|Piano|Inst|Recording|レコーディング|CV|3DCG \/ Composite \/ Motion Graphics|Participant)(?:\s*(?:&|\/|\+)\s*[a-zA-Z0-9\u3000-\u30Fe\u4e00-\u9fa0]+)*\s*[：:\-]\s*(.*)/i;
      ...
      if (item.description_raw) {
        ...
        lines.forEach(line => {
          ...
          if (match) {
            ...
            let key = ROLE_MAPPING[rawKey] || ROLE_MAPPING[Object.keys(ROLE_MAPPING).find(k => k.toLowerCase() === rawKey.toLowerCase())] || rawKey.charAt(0).toUpperCase() + rawKey.slice(1);
